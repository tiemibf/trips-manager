...
func (api *API) PostTrips(w http.ResponseWriter, r *http.Request) *spec.Response {
	...
	if len(body.Destination) < 4 {
		return spec.PostTripsJSON400Response(spec.Error{Message: "destination should have at least 4 characters"})
	}
	...
	go func() {
		if err := api.mailer.SendConfirmTripEmailToTripOwner(tripID); err != nil {
			api.logger.Error("failed to send confirmation email", zap.Error(err), zap.String("trip_id", tripID.String()))
		}
	}()
